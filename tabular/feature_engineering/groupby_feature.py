...
def get_candidate_numerical_feature(df, target, k, method='abs_corr'):
    ...
    if method == 'abs_corr':
        ...
        slect_feature_cols = abs_corr.sort_values(
            ascending=False)[1:k].index.values.tolist()
    else:
...
def groupby_generate_feature(df, target, threshold, k, methods, reserve=False):
    """Automatic generate features by first order groupby using category and numerical features"""
